	...
	$['lz'] = {
		...
		Alert: function(content, title, userSetting) {
			...
			function addLayer() {
				...
				//遮罩
				if (!settings.noMask) {
					var dom1 = settings.noMask ? '' : '<div class="lz-layer"></div>';
					$body.append(dom1);
				}
				...
				//框体
				var dom2 = '<div class="lz-alert" style="display:none;">' + '<h2 class="lz-alert-title">' + settings.title + '<span class="lz-close" style="float:right;cursor:pointer;">关闭</span>' + '</h2>' + '<div class="lz-alert-con">' + settings.content + '</div>' + '<div class="lz-btns">' + btns + '</div>';
				$(".lz-layer").css({
